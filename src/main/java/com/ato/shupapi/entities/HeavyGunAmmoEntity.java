...
public class HeavyGunAmmoEntity extends AbstractAutocannonProjectile {
    public HeavyGunAmmoEntity(EntityType<? extends AbstractAutocannonProjectile> type, Level level) {
        super(type, level);
    }
    ...
    public void onAddedToWorld() {
        ...
        if (!this.level().isClientSide()) {
            SmallShellFireEntity shupapiumProjectile = new SmallShellFireEntity(
                    CrustyChunksModEntities.SMALL_SHELL_FIRE.get(),
                    this.level()
            );
            shupapiumProjectile.setNoGravity(true);
            shupapiumProjectile.setPos(this.getX(), this.getY(), this.getZ());
            shupapiumProjectile.shoot(this.getDeltaMovement().x, this.getDeltaMovement().y, this.getDeltaMovement().z, 3.2F, 0.9F);
