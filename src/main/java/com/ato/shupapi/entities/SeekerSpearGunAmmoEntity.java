...
public class SeekerSpearGunAmmoEntity extends AbstractAutocannonProjectile {
    public SeekerSpearGunAmmoEntity(EntityType<? extends AbstractAutocannonProjectile> type, Level level) {
        super(type, level);
    }
    ...
    public void onAddedToWorld() {
        ...
        if (!this.level().isClientSide()) {
            SeekerSpearMissileProjectileEntity shupapiumProjectile = new SeekerSpearMissileProjectileEntity(
                    CrustyChunksModEntities.SEEKER_SPEAR_MISSILE_PROJECTILE.get(),
                    this.level()
            );
            ...
            shupapiumProjectile.shoot(this.getDeltaMovement().x, this.getDeltaMovement().y, this.getDeltaMovement().z, 12.2F, 1.4F);
