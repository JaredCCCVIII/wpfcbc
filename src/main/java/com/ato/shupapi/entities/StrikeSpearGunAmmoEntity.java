...
public class StrikeSpearGunAmmoEntity extends AbstractAutocannonProjectile {
    public StrikeSpearGunAmmoEntity(EntityType<? extends AbstractAutocannonProjectile> type, Level level) {
        super(type, level);
    }
    ...
    public void onAddedToWorld() {
        ...
        if (!this.level().isClientSide()) {
            StrikeSpearProjectileEntity shupapiumProjectile = new StrikeSpearProjectileEntity(
                    CrustyChunksModEntities.STRIKE_SPEAR_PROJECTILE.get(),
                    this.level()
            );
            ...
            shupapiumProjectile.shoot(this.getDeltaMovement().x, this.getDeltaMovement().y, this.getDeltaMovement().z, 20.0F, 0.6F);
