...
public class BattleArtilleryGasGunAmmoEntity extends AbstractAutocannonProjectile {
    ...
    public void onAddedToWorld() {
        ...
        if (!this.level().isClientSide()) {
            GasArtilleryProjectileEntity shupapiumProjectile = new GasArtilleryProjectileEntity(
                    CrustyChunksModEntities.GAS_ARTILLERY_PROJECTILE.get(),
                    this.level()
            );
            shupapiumProjectile.setPos(this.getX(), this.getY(), this.getZ());
            shupapiumProjectile.shoot(this.getDeltaMovement().x, this.getDeltaMovement().y, this.getDeltaMovement().z, 3.1F, 0.9F);
