...
public class FireSpearGunAmmoItem extends Item implements AutocannonAmmoItem {
    public FireSpearGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.FIRE_SPEAR_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.FIRE_SPEAR_GUN_AMMO.get();
    }
