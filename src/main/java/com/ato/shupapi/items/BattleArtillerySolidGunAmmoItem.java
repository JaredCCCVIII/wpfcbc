...
public class BattleArtillerySolidGunAmmoItem extends Item implements AutocannonAmmoItem {
    public BattleArtillerySolidGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.BATTLE_ARTILLERY_SOLID_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.BATTLE_ARTILLERY_SOLID_GUN_AMMO.get();
    }
    ...
    public ItemStack getSpentItem(ItemStack itemStack) {
        return CrustyChunksModItems.HUGE_PROJECTILE.get().getDefaultInstance();
    }
