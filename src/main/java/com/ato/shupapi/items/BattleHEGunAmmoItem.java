...
public class BattleHEGunAmmoItem extends Item implements AutocannonAmmoItem {
    public BattleHEGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.BATTLE_HE_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.BATTLE_HE_GUN_AMMO.get();
    }
