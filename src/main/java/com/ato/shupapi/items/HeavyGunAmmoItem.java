...
public class HeavyGunAmmoItem extends Item implements AutocannonAmmoItem {
    public HeavyGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.HEAVY_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.HEAVY_GUN_AMMO.get();
    }
    ...
    public ItemStack getSpentItem(ItemStack itemStack) {
        return CrustyChunksModItems.SMALL_HE_PROJECTILE.get().getDefaultInstance();
    }
