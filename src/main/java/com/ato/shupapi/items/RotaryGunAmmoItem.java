...
public class RotaryGunAmmoItem extends Item implements AutocannonAmmoItem {
    public RotaryGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.ROTARY_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.ROTARY_GUN_AMMO.get();
    }
