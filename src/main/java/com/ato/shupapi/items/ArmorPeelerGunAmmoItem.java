...
public class ArmorPeelerGunAmmoItem extends Item implements AutocannonAmmoItem {
    public ArmorPeelerGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.ARMOR_PEELER_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.ARMOR_PEELER_GUN_AMMO.get();
    }
    ...
    public ItemStack getSpentItem(ItemStack itemStack) {
        return CrustyChunksModItems.ARMOR_PEELER_UNLOADED.get().getDefaultInstance();
    }
