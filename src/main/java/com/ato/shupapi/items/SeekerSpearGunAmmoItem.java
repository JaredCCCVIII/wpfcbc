...
public class SeekerSpearGunAmmoItem extends Item implements AutocannonAmmoItem {
    public SeekerSpearGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.SEEKER_SPEAR_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.SEEKER_SPEAR_GUN_AMMO.get();
    }
