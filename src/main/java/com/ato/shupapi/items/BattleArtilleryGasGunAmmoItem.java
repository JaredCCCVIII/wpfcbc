...
public class BattleArtilleryGasGunAmmoItem extends Item implements AutocannonAmmoItem {
    public BattleArtilleryGasGunAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public @Nullable AbstractAutocannonProjectile getAutocannonProjectile(ItemStack itemStack, Level level) {
        return ShupapiumBlockEntities.BATTLE_ARTILLERY_GAS_GUN_AMMO.create(level);
    }

    @Override
    public @Nullable EntityType<?> getEntityType(ItemStack itemStack) {
        return ShupapiumBlockEntities.BATTLE_ARTILLERY_GAS_GUN_AMMO.get();
    }
