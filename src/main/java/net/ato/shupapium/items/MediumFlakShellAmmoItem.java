...
public class MediumFlakShellAmmoItem extends AbstractShupapiumACAmmoItem{
    public MediumFlakShellAmmoItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    protected ResourceLocation propertiesLocation() {
        return new ResourceLocation(MainShupapium.MODID, "munition_properties/projectiles/medium_flak_shell");
    }

    @Override
    public @Nullable List<AbstractArrow> getAutocannonProjectile(ItemStack stack, Level level) {
        return List.of(Objects.requireNonNull(CrustyChunksModEntities.SMALL_FLAK_SHELL_PROJECTILE.get().create(level)));
    }

    @Override
    public @Nullable List<EntityType<?>> getEntityType(ItemStack stack) {
        return List.of(CrustyChunksModEntities.SMALL_FLAK_SHELL_PROJECTILE.get());
    }
