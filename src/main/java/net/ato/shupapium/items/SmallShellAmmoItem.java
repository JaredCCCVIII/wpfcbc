...
public class SmallShellAmmoItem extends AbstractShupapiumACAmmoItem{
    public SmallShellAmmoItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public @Nullable List<AbstractArrow> getAutocannonProjectile(ItemStack stack, Level level) {
        return List.of(Objects.requireNonNull(CrustyChunksModEntities.HUGE_BULLET_FIRE.get().create(level)));
    }

    @Override
    public @Nullable List<EntityType<?>> getEntityType(ItemStack stack) {
        return List.of(CrustyChunksModEntities.HUGE_BULLET_FIRE.get());
    }
