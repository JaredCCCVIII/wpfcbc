...
public class ChaffAmmoItem extends AbstractShupapiumACAmmoItem {
    public ChaffAmmoItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public @Nullable List<AbstractArrow> getAutocannonProjectile(ItemStack stack, Level level) {
        return List.of(Objects.requireNonNull(CrustyChunksModEntities.CHAFF.get().create(level)));
    }

    @Override
    public @Nullable List<EntityType<?>> getEntityType(ItemStack stack) {
        return List.of(CrustyChunksModEntities.CHAFF.get());
    }
