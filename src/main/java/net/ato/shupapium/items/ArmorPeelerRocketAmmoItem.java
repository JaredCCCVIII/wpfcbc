...
public class ArmorPeelerRocketAmmoItem extends AbstractShupapiumACAmmoItem{
    public ArmorPeelerRocketAmmoItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public @Nullable List<AbstractArrow> getAutocannonProjectile(ItemStack stack, Level level) {
        return List.of(Objects.requireNonNull(CrustyChunksModEntities.ROCKET.get().create(level)));
    }

    @Override
    public @Nullable List<EntityType<?>> getEntityType(ItemStack stack) {
        return List.of(CrustyChunksModEntities.ROCKET.get());
    }
